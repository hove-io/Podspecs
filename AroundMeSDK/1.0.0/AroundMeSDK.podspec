 
Pod::Spec.new do |s|
  s.name               = "AroundMeSDK"
  s.version            = '1.0.0'
  s.swift_version      = '5.2'
  s.summary            = "AroundMeSDK for Kisio. Show whats around user within your application."
  s.homepage           = "https://github.com/CanalTP/aroundme_ios"
  s.license            = "License"
  s.author             = { "SDK Team" => "team.sdk@kisio.com" }
  s.source             = { :http => "https://kisiodigital.jfrog.io/kisiodigital/ios-release/aroundme/1.0.0/AroundMeSDK.framework.zip", :sha1 => '8c02b29986b803436632b20d56655db553c0e7c0' }
  s.platform           = :ios, "10.0"

  s.dependency         'NavitiaSDK', '2.1.0'
s.dependency         'Toolbox', '1.3.0'
s.dependency         'DesignEngine', '1.3.0'
s.dependency         'Router', '0.4.1'
  s.dependency         'RealmSwift', '10.17.0'
  s.dependency         'MaterialComponents/BottomSheet'
  
  s.exclude_files       = "AroundMeSDK/AroundMeSDKTests/*"
  s.module_name         = "AroundMeSDK"
  s.vendored_frameworks = 'AroundMeSDK.framework'
end
